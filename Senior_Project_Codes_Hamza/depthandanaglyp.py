...
def generate_depth_map(image_path, save_path=None, save=False):
    """
...
def generate_anaglyph(image_path, depth_map, save_path=None, save=False):
    """
    ...
    # Save if required
    if save and save_path:
        cv2.imwrite(save_path, anaglyph)
        print(f"Anaglyph image saved to {save_path}")

    # Display the anaglyph image
    cv2.imshow("Anaglyph Image", anaglyph)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
